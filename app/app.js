...
const ChatApi = {
  ...
  async sendMessage(channelId, { body, attachments, parentId, mentions } = {}) {
    ...
    const row = { channel_id: channelId, body: text || null, attachments: attachments || [], mentions: mentions || [] };
...
const MyFamilyView = {
  ...
  render() {
    ...
    // v4.69: cards auto-size to their content (group chips, flags, memoriam
    // banners…), so the old fixed-CH bottom anchors could start a connector
    // inside a tall card or leave a gap under a short one. Measure the real
    // rendered height of every card and anchor to that instead.
    const realH = {};
    nodes.querySelectorAll('.node').forEach(el => {
      const h = el.offsetHeight;
      if (h) realH[el.dataset.id] = h;
    });
    const cardH = (id) => realH[id] || CH;
    const ANCHOR_TOP = (id) => {
      const p = rowFor[id];
    ...
    const ANCHOR_BOTTOM = (id) => {
      const p = rowFor[id];
      return { x: p.x + CW / 2 + shiftX, y: p.y + cardH(id) + shiftY };
    };
    ...
    if (kidGroups.length) {
      ...
      kidGroups.forEach(({ ps, kids, _lane }) => {
        ...
        // v4.69: lanes stack UPWARD from 16px above the kid row. The old
        // downward stagger (-40 + lane*18) could push lane 3 into the kid
        // cards themselves; upward lanes can never enter the row.
        const trunkY = kidTops[0].y - 16 - (_lane % 4) * LANE_OFFSET;
        const couple = ps.length === 2 &&
          ps[0].spouseId === ps[1].id && !ps[0].divorced && !ps[1].divorced;

        if (couple) {
          // Bus sits just below the TALLER of the two cards, so it never
          // slices through a card that grew (group chip, flags, …).
          const yLine = Math.max(parentBottoms[0].y, parentBottoms[1].y) + 16;
    ...
    const drawParentHeart = (a, b, divorced) => {
      ...
      const bucket = divorced ? exLines : lines;
      // v4.69: when another parent-row card sits BETWEEN the pair (Tony ↔
      // Mimi with Sunyi in the middle), a straight line would tunnel through
      // that card. Route the connector under the row instead: drop from both
      // card bottoms, join with a low horizontal, heart at the midpoint.
      const rowPeers = [...parents, ...stepParents].filter(o => o.id !== a.id && o.id !== b.id);
      const blocked = rowPeers.some(o => {
        const r = rowFor[o.id]; if (!r) return false;
        const l = r.x + shiftX, rr = r.x + CW + shiftX;
        return rr > leftX + 4 && l < rightX - 4;
      });
      if (blocked) {
        const pa = ANCHOR_BOTTOM(a.id), pb = ANCHOR_BOTTOM(b.id);
        const rowBottom = Math.max(...[...parents, ...stepParents]
          .filter(o => rowFor[o.id])
          .map(o => rowFor[o.id].y + cardH(o.id))) + shiftY;
        const byY = rowBottom + 30;
        bucket.push(`M ${pa.x} ${pa.y} V ${byY}`);
        bucket.push(`M ${pb.x} ${pb.y} V ${byY}`);
        bucket.push(`M ${Math.min(pa.x, pb.x)} ${byY} H ${Math.max(pa.x, pb.x)}`);
        hearts.push(heartMarker((pa.x + pb.x) / 2, byY, divorced));
        return;
      }
      // Adjacent pair — straight heart-line at the mid-height of the SHORTER
      // card so it stays inside both regardless of rendered height.
      const yLine = ra.y + Math.min(cardH(a.id), cardH(b.id)) / 2 + shiftY;
      bucket.push(`M ${leftX} ${yLine} H ${rightX}`);
    ...
    allPartners.forEach(p => {
      ...
      const b = rowFor[p.id];
      if (!a || !b) return;
      const yLine = a.y + Math.min(cardH(focus.id), cardH(p.id)) / 2 + shiftY;
    ...
    if (children.length) {
      ...
      // Drop must clear the TALLEST card in the focus/partner row before the
      // trunk turns horizontal (v4.69: measured heights, not fixed CH).
      const adultRowClear = Math.max(
        rowFor[focus.id].y + cardH(focus.id),
        ...allPartners.map(p => rowFor[p.id].y + cardH(p.id)),
      ) + shiftY + 12;
      groupsByKey.forEach(({ ids, kids }) => {
        ...
        if (ids.length === 2) {
          ...
          const yLine = a.y + Math.min(cardH(ids[0]), cardH(ids[1])) / 2 + shiftY;
          ...
          start = { x: p.x + CW / 2 + shiftX, y: p.y + cardH(ids[0]) + shiftY };
        }
        const childTops = kids.map(c => ANCHOR_TOP(c.id)).filter(Boolean);
        if (!childTops.length) return;
        const dropTo = Math.max(adultRowClear, start.y + 8);
        lines.push(`M ${start.x} ${start.y} V ${dropTo}`);
        const trunkY = childTops[0].y - 20;
    ...
    children.forEach(c => {
      ...
      const a = rowFor[c.id];
      if (!sp || !a) return;
      const yLine = a.y + Math.min(cardH(c.id), cardH(c.spouseId)) / 2 + shiftY;
    ...
    if (grandchildren.length) {
      ...
      grandchildren.forEach(gc => {
        ...
        if (visibleParents.length >= 2) {
          ...
          const yLine = a.y + Math.min(cardH(visibleParents[0]), cardH(visibleParents[1])) / 2 + shiftY;
          startX = (Math.min(a.x, b.x) + CW + Math.max(a.x, b.x)) / 2 + shiftX;
          startY = yLine;
        } else {
          const p = rowFor[visibleParents[0]];
          startX = p.x + CW / 2 + shiftX;
          startY = p.y + cardH(visibleParents[0]) + shiftY;
        }
        const top = ANCHOR_TOP(gc.id);
        // Clear the tallest card in the children row before turning (v4.69).
        const childRowClear = Math.max(
          ...[...children.map(c => c.id), ...childSpouseIds]
            .filter(id => rowFor[id])
            .map(id => rowFor[id].y + cardH(id)),
        ) + shiftY + 12;
        const dropTo = Math.max(childRowClear, startY + 8);
        lines.push(`M ${startX} ${startY} V ${dropTo}`);
        const trunkY = top.y - 20;
...
// Hand-maintained changelog of meaningful shipped changes (see changelog.json).
// v4.69: the full history (~75 KB) moved out of the bundle into
...
function ensureChangelog() {
  if (CHANGELOG.length) return Promise.resolve(CHANGELOG);
  if (!_changelogPromise) {
    _changelogPromise = fetch('changelog.json', { cache: 'no-cache' })
      .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(list => { CHANGELOG = Array.isArray(list) ? list : []; return CHANGELOG; })
      .catch(() => CHANGELOG);
  }
...
const HistoryView = {
  render() {
    ...
    // Version chip paints immediately from the inline APP_VERSION; the full
    // entry list streams in from changelog.json on first visit (v4.69).
    const current = $('#history-current-version');
    if (current) current.textContent = `v${APP_VERSION}`;
    if (!CHANGELOG.length) list.innerHTML = '<p class="muted small">Loading history…</p>';
    ensureChangelog().then((entries) => {
      // User may have navigated away while the fetch was in flight.
      if (Views.current !== 'history') return;
      if (entries.length) {
        ...
        list.innerHTML = '<p class="muted small">History could not be loaded.</p>';
      }
    });
...
const FriendsTabView = {
  ...
  init() {
    ...
    if (search) {
      // v4.69: debounced — full-list re-render per keystroke gets expensive
      // as the archive grows.
      const _searchTimer = { t: null };
      on(search, 'input', () => {
        clearTimeout(_searchTimer.t);
        _searchTimer.t = setTimeout(() => {
...
const MyKidsView = {
  ...
  async resolvePhotoSrc(el) {
    ...
    // v4.69: cap the cache — evict oldest entries past 400 so a long session
    // browsing thousands of photos can't grow the Map unboundedly.
    if (this.signedUrlCache.size >= 400) {
      for (const k of this.signedUrlCache.keys()) {
        this.signedUrlCache.delete(k);
        if (this.signedUrlCache.size < 300) break;
      }
    }
...
const ChatView = {
  ...
  _bindMentionAutocomplete(input, menu) {
    ...
    on(input, 'keydown', (e) => {
      ...
      if (e.key === 'Escape') { this._closeMentionMenu(); }
    });
    on(menu, 'click', (e) => {
  ...
  _updateMentionMenu(input, menu) {
    ...
    if (!m) { this._closeMentionMenu(); return; }
    const frag = m[1].toLowerCase();
    const matches = ChatApi.mentionables().filter(p => p.name.toLowerCase().includes(frag)).slice(0, 6);
    if (!matches.length) { this._closeMentionMenu(); return; }
    this._mention = { input, start: caret - m[0].length, end: caret };
    menu.innerHTML = matches.map(p =>
      `<button type="button" class="chat-mention-item" data-mention-pick="${escape(p.name)}">@${escape(p.name)}</button>`).join('');
    menu.hidden = false;
  },
  ...
  _closeMentionMenu() {
    this._mention = null;
    const m = $('#chat-mention-menu'); if (m) { m.hidden = true; m.innerHTML = ''; }
  },
